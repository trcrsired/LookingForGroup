...
local function scan(state)
	...
	timer:Cancel()
	local function lockdown_callback()
		FriendsFrame:RegisterEvent("WHO_LIST_UPDATE")
		LookingForGroup_CR:UnregisterEvent("PLAYER_REGEN_ENABLED")
	end
	if InCombatLockdown() then
		LookingForGroup_CR:RegisterEvent("PLAYER_REGEN_ENABLED",lockdown_callback)
	else
		lockdown_callback()
	end
end
